use modql::{
	field::HasSeaFields, filter::{FilterGroups, ListOptions}
};
...
use crate::{
	ctx::Ctx, model::{
		Error, ModelManager, Result, base::{
			CommonIden, DbBmc, LIST_LIMIT_DEFAULT, LIST_LIMIT_MAX, prep_fields_for_create, prep_fields_for_update
		}
	}
};
	...
	E: HasSeaFields,
{
	...
	//       We could have a with_txn as function argument if perf is an issue
	// (it should not be)
	...
	MC: DbBmc,
{
	...
	if result as usize != ids.len() {
		Err(Error::EntityNotFound {
			entity: MC::TABLE,
			id: 0, /* Using 0 because multiple IDs could be not found, you
			        * may want to improve error handling here */
		})
	} else {
		Ok(result)
	}

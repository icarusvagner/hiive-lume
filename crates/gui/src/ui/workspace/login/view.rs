use gpui::*;
use gpui_component::{
	Disableable, Icon, IconName, Sizable, StyledExt, WindowExt, button::{Button, ButtonVariants}, form::{field, v_form}, input::{Input, InputState}, label::Label, notification::NotificationType, v_flex
};
...
impl LoginView {
	pub fn view(window: &mut Window, cx: &mut App) -> Entity<Self> {
		cx.new(|cx| {
			let username = cx
				.new(|cx| InputState::new(window, cx).placeholder("admin1234"));
			let password = cx.new(|cx| {
				InputState::new(window, cx).placeholder("********").masked(true)
			});

			Self { username, password, is_loading: false }
		})
	}
	...
	fn _auth_login(&mut self, window: &mut Window, cx: &mut Context<Self>) {
		...
		if self._validate_empty_input(window, cx) {
			...
			window.push_notification(
				(NotificationType::Warning, "Input fields are required"),
				cx,
			);
		}
	}
	...
	fn _form_button(
		...
		form_btn.on_click(
			cx.listener(|this, _, window, cx| this._auth_login(window, cx)),
		)
	}
}
...
impl Render for LoginView {
	fn render(
		...
		v_flex().h_full().w_full().items_center().justify_center().p_5().child(
			div().w(px(420.)).child(
				v_form()
					.large()
					.gap_2()
					.child(field().col_span(2).label_indent(false).child(
						Label::new("Admin Login").text_3xl().font_semibold(),
					))
					.child(
						field()
							.col_span(2)

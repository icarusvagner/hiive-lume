...
use crate::{
	states::main_layout::{ActiveView, ViewState}, ui::workspace::{home::Homeview, login::LoginView}
};
...
impl Workspace {
	pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
		...
		let _subscription =
			vec![cx.observe_global::<ViewState>(move |this, cx| {
	...
	fn render_login(&self, _cx: &mut Context<Self>) -> Stateful<Div> {
		v_flex().id("login-view").h_full().w_full().child(self.login.clone())
	}

...
use tools::{
	b64::{b64u_decode_to_string, b64u_encode}, time::{now_utc, now_utc_plus_sec_str, parse_utc}
};
use uuid::Uuid;

use crate::config::auth_config;
pub use crate::error::{AuthError as Error, Result};
...
impl FromStr for Token {
	...
	fn from_str(token_str: &str) -> std::result::Result<Self, Self::Err> {
		...
		let (ident_b64u, exp_b64u, sign_b64u) =
...
fn validate_token_sign_and_exp(
	...
	// -- Validate signature.
	let new_sign_b64u = token_sign_into_b64u(
		&origin_token.ident,
		&origin_token.exp,
		salt,
		key,
...
fn token_sign_into_b64u(
	...
	// -- Create a Black3 Hasher (not from key because blake3 key is fixed
	// length).
...
mod tests {
	...
	fn test_token_display_ok() -> Result<()> {
		// -- Fixtures
		let fx_token_str = "ZngtaWRlbnQtMDE.MjAyMy0wNS0xN1QxNTozMDowMFo.\
		                    some-sign-b64u-encoded";
		let fx_token = Token {
	...
	fn test_token_validate_web_token_err_expired() -> Result<()> {
		...
		// -- Check
		assert!(
			matches!(res, Err(error::AuthError::Expired)),

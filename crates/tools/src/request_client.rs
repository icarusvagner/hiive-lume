use std::{
	any::type_name, borrow::Cow, error::Error, mem, pin::Pin, sync::{LazyLock, OnceLock}, task::Poll, time::Duration
};
...
use regex::Regex;
use reqwest::{
	header::{HeaderMap, HeaderValue}, redirect
};
...
impl From<reqwest::Client> for ReqwestClient {
	fn from(client: reqwest::Client) -> Self {
			...
			tokio::runtime::Handle::try_current().unwrap_or_else(|_| {
				...
				let runtime = RUNTIME.get_or_init(|| {
					tokio::runtime::Builder::new_multi_thread()
						// Since we now have two executors, let's try to keep
						// our footprint small
						.worker_threads(1)
						.enable_all()
						.build()
		...
		Self { client, handle, proxy: None, user_agent: None }
	}
}
...
pub fn poll_read_buf(
	...
	let n = {
		...
		// Safety: `chunk_mut()` returns a `&mut UninitSlice`, and `UninitSlice`
		// is a transparent wrapper around `[MaybeUninit<u8>]`.
		let dst = unsafe {
			&mut *(dst as *mut _ as *mut [std::mem::MaybeUninit<u8>])
		};
...
fn redact_error(mut error: reqwest::Error) -> reqwest::Error {
	if let Some(url) = error.url_mut()
		...
		&& let Cow::Owned(redacted) =
			REDACT_REGEX.replace_all(query, "key=REDACTED")
	{
	...
	> {
		...
		self.handle
			.spawn(async move {
				...
				let mut builder =
					http::response::Builder::new().status(response.status());
...
mod tests {
	...
	use crate::request_client::ReqwestClient;
	...
	fn test_proxy_uri() {
		...
		let client =
			ReqwestClient::proxy_and_user_agent(Some(proxy.clone()), "test")
				.unwrap();

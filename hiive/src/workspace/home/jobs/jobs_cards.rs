use gpui::*;
use gpui_component::{
	ActiveTheme, WindowExt, button::{Button, ButtonVariants}, h_flex, label::Label, tooltip::Tooltip, v_flex
};

use crate::{
	data::models::jobs_model::JobsModel, workspace::components::card::custom_card
};
	...
	fn render_cards(
		...
		for (index, job) in self.data.clone().iter().enumerate() {
			let content = v_flex()
					...
					v_flex()
						...
						.child(
							Label::new(job.description())
								...
								.text_color(
									cx.theme().foreground.opacity(0.60),
								),
						),
				)
				.child(
				...
				.on_mouse_up(
					...
					cx.listener(|_, _, window, cx| {
						window.open_dialog(cx, |dialog, _, _| {
							dialog.title("Job Description").footer(
								|_, _, _, _| {
									vec![
										Button::new("job-desc-confirm-btn")
											.primary()
											.py_3()
											.label("Update")
											.cursor_pointer()
										...
										Button::new("job-desc-cancel-btn")
											.py_3()
											.label("Close")
											.cursor_pointer()
											.on_click(|_, window, cx| {
				...
				.tooltip(|window, cx| {
					Tooltip::new("Click to show more").build(window, cx)
				});

...
use gpui::*;
use gpui_component::{
	ActiveTheme, IndexPath, Sizable, WindowExt, checkbox::Checkbox, h_flex, label::Label, select::{Select, SelectItem, SelectState}, table::{Column, ColumnSort, Table, TableDelegate, TableEvent, TableState}, v_flex
};
...
impl TableDelegate for TableModeViewDelegate {
	...
	fn render_td(
		...
		match column.key.as_str() {
			"checkbox" => {
				div().child(Checkbox::new("row_checkbox").checked(false))
			}
	...
	fn render_empty(
		...
		div().child(Label::new("Empty data"))
	}
		...
		match self.columns[col_ix].key.as_ref() {
			...
			"candidate-status" => self.rows.sort_by(|a, b| match sort {
				ColumnSort::Descending => {
					b.status().to_string().cmp(&a.status().to_string())
				}
	...
	// fn load_more(&mut self, window: &mut Window, cx: &mut
	// Context<gpui_component::table::TableState<Self>>) {     if self.loading
	// {         return;
	//     }
...
impl TableModeView {
	pub fn new(window: &mut Window, cx: &mut Context<Self>) -> Self {
		...
		let table_state =
			cx.new(|cx| TableState::new(delegate, window, cx).sortable(true));
...
impl Render for TableModeView {
	fn render(
		...
		div().size_full().rounded_xl().child(self.render_table_mode(window, cx))
	}
}
